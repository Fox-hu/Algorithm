...
fun maxSubArrayByDivide(nums: IntArray): Int {
    if (nums.isEmpty()) return 0
    return maxSubArrayByDivide(nums, 0, nums.size)
}

//有三种可能 分别是在数组的左侧、数组的右侧以及左右侧各占一部分
//所以结果就是比较三者的最大值
fun maxSubArrayByDivide(nums: IntArray, begin: Int, end: Int): Int {
    ...
    //比较三种情况下 那种获得的值最大
    return Math.max(leftMax + rightMax, Math.max(maxSubArrayByDivide(nums, begin, mid), maxSubArrayByDivide(nums, mid, end)))
}